...
export default function ProfileScreen() {
  ...
  const checkExpertStatus = async () => {
    ...
    if (currentLead || profile) {
      setIsExpert(true);
      setExpertData(profile || currentLead);
    }
  };
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background, paddingTop: insets.top }]}>
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        {isExpert ? (
          <TouchableOpacity
            style={[styles.expertPanel, { backgroundColor: colors.cardBackground, borderColor: colors.primary }]}
            onPress={() => router.push('/experts-dashboard')}
          >
            <View style={styles.expertPanelHeader}>
              <Ionicons name="briefcase" size={28} color={colors.primary} />
              <View style={styles.expertPanelInfo}>
                <Text style={[styles.expertPanelTitle, { color: colors.text }]}>Panel de Experto</Text>
                <Text style={[styles.expertPanelSubtitle, { color: colors.textSecondary }]}>
                  {expertData?.specialty || expertData?.specializations?.[0] || 'Experto verificado'}
                </Text>
              </View>
          ...
          <TouchableOpacity
            style={[styles.becomeExpertButton, { backgroundColor: colors.primary }]}
            onPress={() => router.push('/experts-register')}
          >
            <Ionicons name="star" size={24} color={colors.background} />
            <Text style={[styles.becomeExpertText, { color: colors.background }]}>
