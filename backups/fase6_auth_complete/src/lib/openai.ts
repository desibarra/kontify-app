...
// 1. Validación de API Key (Build-time safe)
const apiKey = process.env.EXPO_PUBLIC_OPENAI_API_KEY || "";

if (!apiKey) {
    console.warn(
        "⚠️ OpenAI API Key missing during build time.\n" +
        "This is expected for static generation, but will fail at runtime if key is not injected.\n" +
        "Vercel will inject this variable at runtime via Environment Variables dashboard."
    );
}

// 2. Inicialización del Cliente (Versión v4)
export const openai = new OpenAI({
    apiKey: apiKey || "placeholder-key-for-build", // Evitar crash en build time
